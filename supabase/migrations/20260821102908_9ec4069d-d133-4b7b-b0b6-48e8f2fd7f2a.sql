ALTER TABLE public.academy_attempts       ALTER COLUMN scenario_score DROP NOT NULL;
ALTER TABLE public.academy_certifications ALTER COLUMN scenario_score DROP NOT NULL;
...
CREATE OR REPLACE FUNCTION public.academy_cert_settings_guard()
RETURNS trigger
LANGUAGE plpgsql
SET search_path TO 'public'
AS $fn$
DECLARE _err text;
BEGIN
  _err := public.academy_cert_settings_error(NEW.certification_settings);
  IF _err IS NOT NULL THEN
    RAISE EXCEPTION 'Invalid certification settings: %', _err;
  END IF;
  RETURN NEW;
END $fn$;

DROP TRIGGER IF EXISTS trg_academy_cert_settings_guard ON public.academy_modules;
CREATE TRIGGER trg_academy_cert_settings_guard
BEFORE INSERT OR UPDATE OF certification_settings ON public.academy_modules
FOR EACH ROW EXECUTE FUNCTION public.academy_cert_settings_guard();
...
AS $fn$
  SELECT CASE
           WHEN coalesce(m.certification_settings, '{}'::jsonb) = '{}'::jsonb
             THEN public.academy_cert_default_settings()
           ELSE public.academy_cert_default_settings() || m.certification_settings
         END
    FROM public.academy_modules m
   WHERE m.id = _module_id
$fn$;

CREATE OR REPLACE FUNCTION public.academy_cert_blueprint_ok(_ids uuid[], _settings jsonb)
RETURNS boolean
LANGUAGE plpgsql
STABLE
SET search_path TO 'public'
AS $fn$
DECLARE _c jsonb := coalesce(_settings->'constraints', '{}'::jsonb); _tag record; _n int;
BEGIN
  IF coalesce(array_length(_ids,1),0) <> (_settings->>'question_count')::int THEN RETURN false; END IF;

  IF _c ? 'required_tags' THEN
    FOR _tag IN SELECT key, (value#>>'{}')::int AS minimum FROM jsonb_each(_c->'required_tags') LOOP
      SELECT count(*) INTO _n FROM public.academy_questions
       WHERE id = ANY(_ids) AND tags_json ? _tag.key;
      IF _n < _tag.minimum THEN RETURN false; END IF;
    END LOOP;
  END IF;

  IF coalesce((_c->>'min_multiple_select')::int, 0) > 0 THEN
    SELECT count(*) INTO _n FROM public.academy_questions
     WHERE id = ANY(_ids) AND question_type IN ('multiple_select','scenario_multiple_select');
    IF _n < (_c->>'min_multiple_select')::int THEN RETURN false; END IF;
  END IF;

  IF coalesce((_c->>'min_ordering_or_classification')::int, 0) > 0 THEN
    SELECT count(*) INTO _n FROM public.academy_questions
     WHERE id = ANY(_ids) AND question_type IN ('ordering','classification');
    IF _n < (_c->>'min_ordering_or_classification')::int THEN RETURN false; END IF;
  END IF;

  IF coalesce((_c->>'min_hard_or_expert')::int, 0) > 0 THEN
    SELECT count(*) INTO _n FROM public.academy_questions
     WHERE id = ANY(_ids) AND difficulty IN ('hard','expert');
    IF _n < (_c->>'min_hard_or_expert')::int THEN RETURN false; END IF;
  END IF;

  IF coalesce((_c->>'max_per_scenario_group')::int, 0) > 0 THEN
    SELECT coalesce(max(c), 0) INTO _n FROM (
      SELECT count(*) c FROM public.academy_questions
       WHERE id = ANY(_ids) AND scenario_group IS NOT NULL GROUP BY scenario_group
    ) s;
    IF _n > (_c->>'max_per_scenario_group')::int THEN RETURN false; END IF;
  END IF;
...
STABLE
SET search_path TO 'public'
AS $fn$ SELECT public.academy_cert_blueprint_ok(_ids, public.academy_cert_default_settings()) $fn$;
...
AS $fn$
DECLARE
  _s jsonb := public.academy_cert_settings(_module_id);
  _diffs text[]; _grp jsonb; _cats text[]; _picked uuid[]; _ids uuid[]; _try int;
BEGIN
  IF _s IS NULL THEN RAISE EXCEPTION 'Module not found'; END IF;

  IF jsonb_typeof(_s->'allowed_difficulties') = 'array' THEN
    SELECT array_agg(value) INTO _diffs FROM jsonb_array_elements_text(_s->'allowed_difficulties');
  END IF;
  ...
  FOR _try IN 1..200 LOOP
    ...
    FOR _grp IN SELECT * FROM jsonb_array_elements(_s->'blueprint') LOOP
      SELECT array_agg(value) INTO _cats FROM jsonb_array_elements_text(_grp->'categories');

      SELECT coalesce(array_agg(p.id), ARRAY[]::uuid[]) INTO _picked FROM (
        SELECT q.id FROM public.academy_questions q
         WHERE q.module_id = _module_id
           AND q.status = 'published'
           AND q.category = ANY(_cats)
           AND (_diffs IS NULL OR q.difficulty = ANY(_diffs))
           AND NOT (q.id = ANY(_ids))
         ORDER BY random()
         LIMIT (_grp->>'count')::int
      ) p;

      _ids := _ids || _picked;
    END LOOP;

    IF coalesce(array_length(_ids,1),0) = (_s->>'question_count')::int
       AND public.academy_cert_blueprint_ok(_ids, _s)
       AND (
...
DECLARE
  ...
  _s jsonb; _pass_score numeric; _scen_pass numeric; _waits jsonb; _hours numeric;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  ...
  _scen := CASE WHEN _scen_pass IS NULL THEN NULL
                ...
                ELSE 0 END;
  _passed := (_weighted >= _pass_score) AND (_scen_pass IS NULL OR _scen >= _scen_pass);
...
DECLARE
  ...
  _next timestamptz; _state text; _s jsonb;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
...
UPDATE public.academy_modules
 ...
 WHERE slug = 'module-5-qualification'
   AND coalesce(certification_settings, '{}'::jsonb) = '{}'::jsonb;

DO $mig$
DECLARE _m record; _next_sort int; _n int;
BEGIN
  SELECT * INTO _m FROM public.academy_modules WHERE slug = 'module-1-welcome-to-manwinwin';
  ...
  IF NOT EXISTS (SELECT 1 FROM public.academy_missions
                  WHERE module_id = _m.id AND item_kind = 'certification') THEN
