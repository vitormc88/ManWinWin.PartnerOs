...
RETURNS TABLE (
  ...
  active_renewal_count int,
  year1_value numeric,
  recurring_arr numeric,
  one_time_value numeric,
  next_renewal_date date,
  next_renewal_value numeric,
  days_to_renewal int,
  has_license boolean,
  ...
  recurring_items jsonb,
  one_time_items jsonb,
  not_renewed_items jsonb,
  missing_modules jsonb,
  missing_plugins jsonb,
  proposed_not_purchased jsonb,
  upsell_opportunities jsonb,
  risk_signals jsonb,
  recommended_actions jsonb,
  commercial_score int,
  expansion_potential numeric,
  high_confidence_potential numeric,
  medium_confidence_potential numeric,
  low_confidence_potential numeric,
  renewal_risk text,
  confidence text,
...
BEGIN
  ...
  BEGIN
    v_partner_uuid := nullif(c_row.partner_id, '')::uuid;
  EXCEPTION WHEN others THEN
    v_partner_uuid := NULL;
  END;

  IF v_partner_uuid IS NOT NULL THEN
    SELECT name INTO v_partner_name FROM public.partners WHERE id = v_partner_uuid;
  END IF;
  ...
  SELECT
    coalesce(sum(CASE WHEN cl.billing_frequency IN ('yearly','monthly','per-user-month','annual') THEN cl.amount ELSE 0 END),0),
    coalesce(sum(CASE WHEN cl.billing_frequency IS NULL OR cl.billing_frequency IN ('one-time','one_time') THEN cl.amount ELSE 0 END),0)
  INTO v_recurring, v_one_time
  ...
  WHERE cl.client_id = client_uuid
    AND cl.billing_frequency IN ('yearly','monthly','per-user-month','annual');
  ...
  FROM public.contract_lines cl
  WHERE cl.client_id = client_uuid
    AND (cl.billing_frequency IS NULL OR cl.billing_frequency IN ('one-time','one_time'));
...
GRANT EXECUTE ON FUNCTION public.get_client_commercial_intelligence(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_client_commercial_intelligence(uuid) TO service_role;

DROP VIEW IF EXISTS public.client_commercial_intelligence;
CREATE VIEW public.client_commercial_intelligence
WITH (security_invoker = true) AS
SELECT i.*
FROM public.clients c
CROSS JOIN LATERAL public.get_client_commercial_intelligence(c.id) AS i;

GRANT SELECT ON public.client_commercial_intelligence TO authenticated;
GRANT SELECT ON public.client_commercial_intelligence TO service_role;
