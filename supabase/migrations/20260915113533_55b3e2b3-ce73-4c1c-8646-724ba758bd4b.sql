...
-- Additive, idempotent integration foundation for SharpSpring -> PartnerOS sync (TEST)

ALTER TABLE public.deals
  ADD COLUMN IF NOT EXISTS source_system text,
  ADD COLUMN IF NOT EXISTS external_opportunity_id text,
  ADD COLUMN IF NOT EXISTS external_stage_id text,
  ADD COLUMN IF NOT EXISTS external_stage_name text,
  ADD COLUMN IF NOT EXISTS source_owner_id text,
...
CREATE UNIQUE INDEX IF NOT EXISTS deals_source_external_opportunity_uidx
  ON public.deals (lower(source_system), external_opportunity_id)
  WHERE source_system IS NOT NULL AND external_opportunity_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS deal_activities_source_external_note_uidx
  ON public.deal_activities (lower(source_system), external_note_id)
  WHERE source_system IS NOT NULL AND external_note_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS deal_contacts_source_external_contact_uidx
  ON public.deal_contacts (lower(source_system), external_contact_id)
  WHERE source_system IS NOT NULL AND external_contact_id IS NOT NULL;
...
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_array_elements(payload) LOOP
    ...
    IF ext_id IS NULL THEN errors := errors || 'missing external_opportunity_id'; END IF;
    IF company IS NULL THEN errors := errors || 'missing company_name'; END IF;

    amount := NULL;
    BEGIN
      amount := (COALESCE(item->>'amount_eur', item->>'amount', '0'))::numeric;
    EXCEPTION WHEN others THEN
      errors := errors || 'amount_eur is not numeric';
    END;
    IF amount IS NOT NULL AND amount < 0 THEN errors := errors || 'amount_eur must be >= 0'; END IF;

    IF close_date_txt IS NOT NULL THEN
      BEGIN
        PERFORM close_date_txt::date;
      EXCEPTION WHEN others THEN
        errors := errors || 'close_date is not a valid date';
      END;
    END IF;
    ...
    IF mapped_stage IS NULL THEN
      errors := errors || format('unrecognized source stage: %s', COALESCE(src_stage,'(none)'));
    ELSE
      warnings := warnings || evidence_warning;
      IF mapped_stage = 'Won' THEN
        warnings := warnings || 'Won/Closed Won: operational conversion is pending a valid PartnerOS proposal. No client, license, contract, revenue or renewal record is created by this dry-run.';
      END IF;
    END IF;
    ...
    ELSE
      ...
      ELSE
        ...
        ELSIF array_length(candidates,1) = 1 THEN
          action := 'review';
          warnings := warnings || 'Candidate PartnerOS deal matched by normalized company/email. Ambiguous match is never merged automatically.';
        ELSE
          action := 'review';
          warnings := warnings || 'Multiple candidate PartnerOS deals matched by normalized company/email. Manual review required.';
        END IF;
      END IF;
    END IF;
