...
BEGIN
  ...
  LOOP
    ...
    -- placeholder bodies), so the block is left exactly where it is.
    SELECT count(*) INTO v_head_cnt
    FROM regexp_matches(v_md, '(?n)^' || regexp_replace(v_row.heading, '([.^$*+?()\[\]{}|\\-])', '\\\1', 'g') || '\s*$', 'g');
    ...
    --    tolerating any extra directive lines inside the block.
    ...
    -- 3. Re-insert exactly once, immediately after the intended heading.
    v_new := regexp_replace(
      v_new,
      '(?n)^(' || regexp_replace(v_row.heading, '([.^$*+?()\[\]{}|\\-])', '\\\1', 'g') || '\s*\n)',
      '\1' || E'\n' || v_fence || E'\n'
    );
    ...
    IF v_new !~ ('(?n)^' || regexp_replace(v_row.heading, '([.^$*+?()\[\]{}|\\-])', '\\\1', 'g')
